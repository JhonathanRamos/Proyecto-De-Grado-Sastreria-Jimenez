                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h1 class="card-title mb-0">Reporte: Detalles de Ventas</h1>
    ...
    <i class="mdi mdi-printer"></i> Imprimir
                            ...
                            <div class="table-responsive">
                                <table class="table table-dark table-hover sortable" id="dataTable">
                                    ...
                                    <tbody>
                                        <?php if (isset($detallesVentas) && !empty($detallesVentas)): ?>
                                            <?php foreach ($detallesVentas as $detalle): ?>
                                                <tr>
                                                    <td><?= $detalle['idDetalleVenta'] ?></td>
                                                    <td><?= $detalle['cliente'] . ' ' . $detalle['apellido'] ?></td>
                                                    <td><?= $detalle['producto'] ?></td>
                                                    <td><?= $detalle['cantidad'] ?></td>
                                                    <td><?= number_format($detalle['precio_unitario'], 2) ?> Bs</td>
                                                    <td><?= number_format($detalle['subtotal'], 2) ?> Bs</td>
                                                    <td><?= number_format($detalle['descuento'], 2) ?> Bs</td>
                                                    <td><?= number_format($detalle['subtotal'] - $detalle['descuento'], 2) ?> Bs</td>
                                                </tr>
                                            <?php endforeach; ?>
                                        <?php else: ?>
                                            <tr>
...
function sortTable(columnIndex) {
    ...
    filteredData.sort((a, b) => {
        ...
        // Remover el símbolo Bs y convertir a número si es un valor monetario
        if (valueA.includes('Bs')) {
            valueA = parseFloat(valueA.replace('Bs', '').trim());
            valueB = parseFloat(valueB.replace('Bs', '').trim());
        }
        // Si no es valor monetario, intentar convertir a número
...
function handlePrint() {
    ...
    printContent.innerHTML = `
        <div class="print-header">
            ...
            <h3>Reporte de Detalles de Ventas</h3>
    ...
    printFrame.contentDocument.write(`
        <html>
            <head>
                <title>Reporte de Detalles de Ventas - Sastrería Jimenez</title>
                <style>

                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                <h1 class="card-title mb-0">Reporte: Ventas por Método de Pago</h1>
                            ...
                            <div class="table-responsive">
                                <table class="table table-dark table-hover sortable" id="dataTable">
                                    ...
                                    <tbody>
                                        <?php 
                                        if (isset($ventasPorMetodo) && !empty($ventasPorMetodo)):
                                            $totalVentasGeneral = array_sum(array_column($ventasPorMetodo, 'monto_total'));
                                        ?>
                                            <?php foreach ($ventasPorMetodo as $metodo): ?>
                                                <tr>
                                                    <td><?= $metodo['metodoPago'] ?? 'No especificado' ?></td>
                                                    <td><?= $metodo['total_ventas'] ?></td>
                                                    <td><?= number_format($metodo['monto_total'], 2) ?> Bs</td>
                                                    <td><?= number_format($metodo['monto_total'] / $metodo['total_ventas'], 2) ?> Bs</td>
                                                    <td><?= number_format(($metodo['monto_total'] / $totalVentasGeneral) * 100, 1) ?>%</td>
                                                </tr>
                                            <?php endforeach; ?>
                                            <tr class="bg-primary total-row">
                                                <td><strong>Total General</strong></td>
                                                <td><strong><?= array_sum(array_column($ventasPorMetodo, 'total_ventas')) ?></strong></td>
                                                <td><strong><?= number_format($totalVentasGeneral, 2) ?> Bs</strong></td>
                                                <td><strong><?= number_format($totalVentasGeneral / array_sum(array_column($ventasPorMetodo, 'total_ventas')), 2) ?> Bs</strong></td>
                            ...
                            <div class="mt-4 no-print">
                                <h3 class="card-title">Distribución de Ventas por Método de Pago</h3>
...
function sortTable(columnIndex) {
    ...
    filteredData.sort((a, b) => {
        ...
        // Remover símbolos y convertir a número si es posible
        if (valueA.includes('Bs') || valueA.includes('%')) {
            valueA = parseFloat(valueA.replace(/[Bs%]/g, '').trim());
            valueB = parseFloat(valueB.replace(/[Bs%]/g, '').trim());
        }
...
function initializeChart() {
    <?php if (isset($ventasPorMetodo) && !empty($ventasPorMetodo)): ?>
    ...
    var myChart = new Chart(ctx, {
        ...
        data: {
            labels: [
                <?php foreach ($ventasPorMetodo as $metodo): ?>
                '<?= $metodo['metodoPago'] ?? 'No especificado' ?>',
                <?php endforeach; ?>
            ],
            datasets: [{
                data: [
...
function handlePrint() {
    ...
    printContent.innerHTML = `
        <div class="print-header">
            ...
            <h3>Reporte de Ventas por Método de Pago</h3>
    ...
    printContent.innerHTML += `
        ...
        <!-- Agregar el gráfico para la impresión -->
        <div style="margin-top: 30px; text-align: center;">
            <h3>Distribución de Ventas</h3>
            <img src="${document.getElementById('ventasChart').toDataURL()}" />
        </div>
    `;
    ...
    printFrame.contentDocument.write(`
        <html>
            <head>
                <title>Reporte de Ventas por Método de Pago - Sastrería Jimenez</title>
                <style>
                    body { 
